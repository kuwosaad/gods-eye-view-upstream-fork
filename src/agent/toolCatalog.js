/**
 * Canonical God’s Eye View agent tool catalog.
 *
 * This dependency-free catalog is shared by every agent transport. Keep tool
 * names, descriptions, and JSON schemas here so adapters cannot drift.
 */
const GEV_AGENT_TOOL_CATALOG = [
  {
    type: 'function',
...
/**
 * Transport-neutral policy metadata. Keep this separate from the wire
 * schemas: OpenAI Realtime and MCP must receive the same 28 tool objects.
 *
 * costClass uses the server security policy vocabulary. free means local
 * state/UI work, network may contact a provider, artifact produces a bounded
 * file or image, and expensive may load/query substantial live data.
 */
const TOOL_METADATA = {
  fly_to_location: ['mutation', 'camera', 'network'],
  select_nearest_aircraft: ['mutation', 'aircraft', 'expensive'],
  adjust_camera_zoom: ['mutation', 'camera', 'free'],
  zoom_to_globe: ['mutation', 'camera', 'free'],
  set_layer_visibility: ['mutation', 'layers', 'network'],
  show_data_layers_menu: ['mutation', 'ui', 'free'],
  set_panel_open: ['mutation', 'ui', 'free'],
  set_context_mode: ['mutation', 'context', 'network'],
  control_cockpit: ['mutation', 'cockpit', 'free'],
  set_visual_style: ['mutation', 'visual-style', 'free'],
  get_entity_context: ['read-only', 'observation', 'expensive'],
  get_current_view_state: ['read-only', 'observation', 'free'],
  set_hud: ['mutation', 'visual-style', 'free'],
  set_detection: ['mutation', 'visual-style', 'free'],
  set_map_stack: ['mutation', 'visual-style', 'network'],
  set_post_processing: ['mutation', 'visual-style', 'free'],
  control_scene: ['mutation', 'scene', 'free'],
  control_cctv: ['mutation', 'cctv', 'network'],
  control_radio: ['mutation', 'radio', 'network'],
  track_entity: ['mutation', 'tracking', 'free'],
  stop_tracking: ['mutation', 'tracking', 'free'],
  frame_overhead: ['mutation', 'camera', 'free'],
  annotate_map: ['mutation', 'annotations', 'network'],
  clear_annotations: ['mutation', 'annotations', 'free'],
  move_camera: ['mutation', 'camera', 'free'],
  fly_route: ['mutation', 'camera', 'free'],
  analyst_query: ['read-only', 'analysis', 'expensive'],
  next_iss_pass: ['read-only', 'orbital-analysis', 'network'],
};

const metadataEntries = Object.entries(TOOL_METADATA);
const catalogNames = new Set(GEV_AGENT_TOOL_CATALOG.map((tool) => tool.name));
if (
  metadataEntries.length !== GEV_AGENT_TOOL_CATALOG.length ||
  metadataEntries.some(([name]) => !catalogNames.has(name))
) {
  throw new Error('GEV tool metadata must classify every catalog tool exactly once');
}

const GEV_AGENT_TOOL_METADATA = Object.freeze(
  Object.fromEntries(
    metadataEntries.map(([name, [access, capability, costClass]]) => [
      name,
      Object.freeze({
        access,
...
function getAgentToolMetadata(name) {
  return GEV_AGENT_TOOL_METADATA[name];
}

function isAgentToolReadOnly(name) {
  return getAgentToolMetadata(name)?.readOnly === true;
}
